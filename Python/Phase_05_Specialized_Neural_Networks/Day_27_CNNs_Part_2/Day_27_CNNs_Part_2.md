...
|:---|:---:|---:|
| [← Day 26: CNNs Part 1](../Day_26_CNNs_Part_1/Day_26_CNNs_Part_1.md) | [All 50 Days Overview](../../README.md) | [Day 28: RNNs →](../Day_28_RNNs/Day_28_RNNs.md) |
...
Solidify your mastery of pooling mechanics, receptive fields, and residual skip connections!

### 🏋️ Problem 1: Hand-Calculating Max Pooling & Global Average Pooling
Consider this $4 \times 4$ single-channel feature map generated by a convolutional layer:

$$X = \begin{bmatrix} 12 & 20 & 30 & 0 \\ 8 & 16 & 4 & 2 \\ 5 & 7 & 18 & 22 \\ 9 & 3 & 40 & 14 \end{bmatrix}$$

**Your Tasks:**
1. Compute the $2 \times 2$ **Max Pooling** output using non-overlapping $2 \times 2$ windows (stride $S = 2$).
2. Compute the **Global Average Pooling (GAP)** scalar output for this entire $4 \times 4$ feature map.
3. How many trainable weight parameters did these two operations introduce?

---

### 🏋️ Problem 2: Mathematical Proof of ResNet Skip Connections
In a standard deep neural network layer without skip connections:
$$y = \mathcal{F}(x)$$
During backpropagation, the gradient is:
$$\frac{\partial \mathcal{L}}{\partial x} = \frac{\partial \mathcal{L}}{\partial y} \cdot \frac{\partial \mathcal{F}}{\partial x}$$
If $\frac{\partial \mathcal{F}}{\partial x} < 1$, multiplying 100 layers together causes the gradient to vanish to $0.0$.

In a **Residual Block (ResNet)**:
$$y = \mathcal{F}(x) + x$$

**Your Tasks:**
1. Compute the derivative $\frac{\partial y}{\partial x}$ for the residual block.
2. Substitute this into the chain rule for $\frac{\partial \mathcal{L}}{\partial x}$.
3. Explain why the gradient can **never vanish completely**, even if the learned sub-network $\mathcal{F}$ has $\frac{\partial \mathcal{F}}{\partial x} \approx 0$.
4. Where is this exact skip connection used in modern Generative AI (LLMs & Diffusion)?

<details>
<summary><b>🔍 Click to Reveal Step-by-Step Solutions</b></summary>

### Solution 1:
1. **$2 \times 2$ Max Pooling:**
   - Top-Left Quadrant: $\max(12, 20, 8, 16) = \mathbf{20}$
   - Top-Right Quadrant: $\max(30, 0, 4, 2) = \mathbf{30}$
   - Bottom-Left Quadrant: $\max(5, 7, 9, 3) = \mathbf{9}$
   - Bottom-Right Quadrant: $\max(18, 22, 40, 14) = \mathbf{40}$
   $$\text{Max Pool Output} = \begin{bmatrix} \mathbf{20} & \mathbf{30} \\ \mathbf{9} & \mathbf{40} \end{bmatrix}$$

2. **Global Average Pooling:**
   Sum of all 16 values:
   $$\text{Sum} = (12+20+30+0) + (8+16+4+2) + (5+7+18+22) + (9+3+40+14) = 62 + 30 + 52 + 66 = 210$$
   $$\text{GAP Output} = \frac{210}{16} = \mathbf{13.125}$$

3. **Trainable Parameters:**
   **Zero ($0$)!** Pooling layers have strictly fixed routing algorithms (taking maximums or averages); they have no learnable weights or biases.

---

### Solution 2:
1. **Derivative $\frac{\partial y}{\partial x}$:**
   $$\frac{\partial y}{\partial x} = \frac{\partial}{\partial x}[\mathcal{F}(x) + x] = \mathbf{\frac{\partial \mathcal{F}}{\partial x} + 1.0}$$

2. **Chain Rule Gradient:**
   $$\frac{\partial \mathcal{L}}{\partial x} = \frac{\partial \mathcal{L}}{\partial y} \left( \frac{\partial \mathcal{F}}{\partial x} + 1.0 \right) = \mathbf{\frac{\partial \mathcal{L}}{\partial y} \frac{\partial \mathcal{F}}{\partial x} + \frac{\partial \mathcal{L}}{\partial y}}$$

3. **Why Gradients Cannot Vanish:**
   Look at the expanded term: even if the layer weights degenerate and $\frac{\partial \mathcal{F}}{\partial x} \to 0$, the gradient expression retains the uninterrupted term $+ \frac{\partial \mathcal{L}}{\partial y} \cdot 1.0$! The error signal flows backward through the identity shortcut directly to earlier layers with zero decay.

4. **Modern GenAI Relevance:**
   This identical residual connection is the backbone of **Transformer blocks** in GPT-4, Claude, and LLaMA:
   $$x_{l+1} = \text{LayerNorm}(x_l + \text{MultiHeadAttention}(x_l))$$
   Without He et al.'s skip connection, 100-layer Large Language Models and U-Net Diffusion models would be impossible to train!
</details>
